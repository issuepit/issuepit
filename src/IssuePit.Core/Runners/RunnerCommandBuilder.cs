...
public static class RunnerCommandBuilder
{
    ...
    public static string BuildArgs(Agent agent, Issue issue, IReadOnlyList<IssueComment>? comments = null)
    {
        ...
        var task = BuildTaskPrompt(issue, comments, agent.Name);
    ...
    /// Usage: opencode run [--agent NAME] [--session ID --fork] [--model MODEL] --format json TASK
    /// https://opencode.ai/docs/cli/#run-1
    /// </summary>
    private static string BuildOpenCodeArgs(Agent agent, string task)
    {
        var args = new StringBuilder("run");
        var agentName = ResolveOpenCodeAgentName(agent);
        if (!string.IsNullOrWhiteSpace(agentName))
            args.Append($" --agent {EscapeShellArg(agentName)}");
    ...
    /// <para>
    /// When <paramref name="agentName"/> is provided, a context preamble identifying the agent
    /// and the IssuePit issue it is working on is prepended to the prompt.
    /// </para>
    /// </summary>
    public static string BuildTaskPrompt(Issue issue, IReadOnlyList<IssueComment>? comments = null, string? agentName = null)
    {
        var sb = new StringBuilder();

        // Agent context preamble — gives the agent its identity and the IssuePit issue reference.
        if (!string.IsNullOrWhiteSpace(agentName))
            sb.AppendLine($"You are the **{agentName}** agent working on the issue described below.");
        else
            sb.AppendLine("You are an IssuePit agent working on the issue described below.");

        sb.AppendLine($"IssuePit issue #{issue.Number} (project {issue.ProjectId}, issue {issue.Id}).");
        sb.AppendLine();
    ...
    /// <summary>
    /// Resolves the opencode agent name for the <c>--agent</c> CLI flag.
    /// For shell/wrapper agents, returns <see cref="Agent.OpenCodeAgentName"/> directly.
    /// For regular agents, normalises the IssuePit display name to match the opencode config key
    /// generated by the container entrypoint (lowercase, spaces replaced with hyphens).
    /// Returns <c>null</c> when no meaningful name can be derived.
    /// </summary>
    public static string? ResolveOpenCodeAgentName(Agent agent)
    {
        // Shell/wrapper agents carry an explicit opencode agent name.
        if (agent.IsShellAgent && !string.IsNullOrWhiteSpace(agent.OpenCodeAgentName))
            return agent.OpenCodeAgentName;

        // Regular agents: derive the key from the display name (same logic as entrypoint.sh).
        if (string.IsNullOrWhiteSpace(agent.Name))
            return null;
        return NormalizeAgentName(agent.Name);
    }

    /// <summary>
    /// Normalises an agent display name into the key format expected by opencode config:
