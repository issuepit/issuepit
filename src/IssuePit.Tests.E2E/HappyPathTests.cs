...
public class HappyPathTests : IAsyncLifetime
{
    ...
    /// <summary>
    /// UI E2E test for Gantt bar navigation: create a milestone via API with start+due dates → navigate to milestones
    /// → switch to Gantt view → click the Gantt bar label → verify the detail page is shown.
    /// </summary>
    [Fact]
    public async Task Ui_HappyPath_GanttBarNavigatesToDetail()
    {
        ...
        var orgSlug = $"ui-ms-gantt-{Guid.NewGuid():N}"[..16];
        var orgResp = await apiClient.PostAsJsonAsync("/api/orgs", new { name = "UI Gantt Nav Org", slug = orgSlug });
        var org = await orgResp.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
        var orgId = Guid.Parse(org.GetProperty("id").GetString()!);

        var projectSlug = $"ui-g-{Guid.NewGuid():N}"[..14];
        var projResp = await apiClient.PostAsJsonAsync("/api/projects",
            new { name = "UI Gantt Nav Project", slug = projectSlug, orgId });
        var project = await projResp.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
        var projectId = project.GetProperty("id").GetString()!;

        // Create milestone with explicit dates so it appears as a Gantt bar
        const string milestoneTitle = "UI E2E Gantt Sprint";
        var msResp = await apiClient.PostAsJsonAsync($"/api/projects/{projectId}/milestones",
            new { title = milestoneTitle, startDate = "2026-01-01", dueDate = "2026-06-30" });
        msResp.EnsureSuccessStatusCode();
        var ms = await msResp.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();
        var milestoneId = ms.GetProperty("id").GetString()!;
        ...
        try
        {
            ...
            await milestonesPage.SwitchToGanttViewAsync();

            // Click the milestone label in the Gantt label column
