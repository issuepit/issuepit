<template>
  <div class="bg-gray-900 border rounded-lg p-3 group transition-all hover:shadow-lg"
    :class="todo.isCompleted ? 'border-gray-800/50 opacity-60' : 'border-gray-800 hover:border-gray-700'">
    <div class="flex items-start gap-2">
      <!-- Checkbox -->
      <button @click.stop="$emit('toggle')"
        :class="['w-4 h-4 mt-0.5 rounded border flex-shrink-0 flex items-center justify-center transition-colors',
      ...
      <div class="flex-1 min-w-0">
        <p class="text-sm text-gray-200 truncate" :class="todo.isCompleted ? 'line-through text-gray-500' : ''">
          {{ todo.title }}
        </p>
        <div class="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          <span v-if="todo.dueDate"
            :class="['text-xs flex items-center gap-1', isOverdue ? 'text-red-400' : 'text-gray-500']">
          ...
          <span v-if="todo.recurringInterval !== TodoRecurringInterval.None"
            ...
            {{ TodoRecurringIntervalLabels[todo.recurringInterval] }}
          </span>
        </div>
      </div>
      ...
      <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <button @click.stop="$emit('edit')" class="p-1 text-gray-600 hover:text-gray-300 transition-colors" title="Edit">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
          </svg>
        </button>
        <button @click.stop="$emit('delete')" class="p-1 text-gray-600 hover:text-red-400 transition-colors" title="Delete">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
