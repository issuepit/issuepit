    ...
    ILogger<GitHubSyncService> logger)
{
    ...
    /// When <paramref name="dryRun"/> is true, no data is persisted — only log output is written.
    /// </summary>
    public async Task<GitHubSyncRun> SyncAsync(Guid projectId, bool dryRun = false, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (dryRun)
                await AppendLogAsync(run, GitHubSyncLogLevel.Info, "[DRY RUN] No changes will be persisted.", ct);
            ...
            var (ghIssues, issuesFetchError) = await FetchAllGitHubIssuesAsync(token, repo, run, ct);

            if (issuesFetchError)
            {
                await FailRunAsync(run, $"GitHub API error while fetching issues. Check logs above for details.", ct);
                return run;
            }

            await AppendLogAsync(run, GitHubSyncLogLevel.Info, $"Fetched {ghIssues.Count} issue(s) from GitHub.", ct);

            // Import oldest issues first so they get the lowest IssuePit numbers.
            ghIssues.Sort((a, b) => a.Number.CompareTo(b.Number));

            int imported = 0, updated = 0, pushed = 0, skipped = 0;

            // Compute maxNumber once outside the loop to avoid duplicate issue numbers.
            ...
            foreach (var ghIssue in ghIssues)
            {
                ...
                if (existing is null)
                {
                    ...
                    imported++;
                    await AppendLogAsync(run, GitHubSyncLogLevel.Info,
                        $"  {(dryRun ? "[DRY RUN] Would import" : "Imported")}: {repo}#{ghIssue.Number} \"{TruncateTitle(ghIssue.Title)}\"", ct);
                }
                ...
                else
                {
                    ...
                    if (!string.Equals(existing.GitHubIssueUrl, ghIssue.HtmlUrl, StringComparison.Ordinal))
                    {
                        ...
                        updated++;
                        await AppendLogAsync(run, GitHubSyncLogLevel.Info,
                            $"  {(dryRun ? "[DRY RUN] Would update link" : "Updated link")}: {repo}#{ghIssue.Number} \"{TruncateTitle(ghIssue.Title)}\"", ct);
                    }
                    else
                    {
                        skipped++;
            ...
            await AppendLogAsync(run, GitHubSyncLogLevel.Info, $"Fetching pull requests from {repo}...", ct);
            var (ghPrs, prsFetchError) = await FetchAllGitHubPrsAsync(token, repo, run, ct);

            if (prsFetchError)
            {
                await AppendLogAsync(run, GitHubSyncLogLevel.Warn, "GitHub API error while fetching pull requests. Skipping PR import.", ct);
            }
            else
            {
                await AppendLogAsync(run, GitHubSyncLogLevel.Info, $"Fetched {ghPrs.Count} pull request(s) from GitHub.", ct);

                // Import oldest PRs first.
                ghPrs.Sort((a, b) => a.Number.CompareTo(b.Number));
                ...
                foreach (var ghPr in ghPrs)
                {
                    ...
                    if (existingMr is null)
                    {
                        ...
                        prsImported++;
                        await AppendLogAsync(run, GitHubSyncLogLevel.Info,
                            $"  {(dryRun ? "[DRY RUN] Would import PR" : "Imported PR")}: {repo}#{ghPr.Number} \"{TruncateTitle(ghPr.Title)}\"", ct);
                    }
                    else
                    {
                        prsSkipped++;
                ...
                await AppendLogAsync(run, GitHubSyncLogLevel.Info,
                    $"PRs: {prsImported} imported, {prsSkipped} unchanged.", ct);

                if (!dryRun && prsImported > 0)
    ...
    public async Task<List<GitHubConflict>> GetConflictsAsync(Guid projectId, CancellationToken ct = default)
    {
        ...
        var (ghIssues, _) = await FetchAllGitHubIssuesAsync(token, repo, null, ct);
    ...
    private async Task<(List<GitHubIssueDto> issues, bool hasError)> FetchAllGitHubIssuesAsync(
        string token, string repo, GitHubSyncRun? run, CancellationToken ct)
    {
        ...
        return (allIssues, hasError);
    }

    private async Task<(List<GitHubPrDto> prs, bool hasError)> FetchAllGitHubPrsAsync(
        string token, string repo, GitHubSyncRun? run, CancellationToken ct)
    {
        var (owner, repoName) = ParseRepo(repo);
        var client = CreateHttpClient(token);

        var allPrs = new List<GitHubPrDto>();
        var page = 1;
        bool hasError = false;
        ...
        while (true)
        {
            ...
            {
                await AppendLogAsync(run, GitHubSyncLogLevel.Error,
                    $"GitHub API returned {(int)response.StatusCode} for PRs page {page}.", ct);
                hasError = true;
                break;
            }

            var prs = await response.Content.ReadFromJsonAsync<List<GitHubPrDto>>(ct) ?? [];
            allPrs.AddRange(prs);
    ...
    /// <summary>
    /// Normalizes a GitHub repository identifier to <c>owner/repo</c> format.
    /// Accepts both <c>owner/repo</c> and full URLs like <c>https://github.com/owner/repo</c>.
    /// </summary>
    private static string NormalizeRepo(string repo)
    {
        repo = repo.Trim().TrimEnd('/');

        // Strip https://github.com/ or http://github.com/ prefix.
        const string httpsPrefix = "https://github.com/";
        const string httpPrefix = "http://github.com/";

        if (repo.StartsWith(httpsPrefix, StringComparison.OrdinalIgnoreCase))
            return repo[httpsPrefix.Length..];

        if (repo.StartsWith(httpPrefix, StringComparison.OrdinalIgnoreCase))
            return repo[httpPrefix.Length..];
    ...
    private static MergeRequestStatus MapGitHubPrState(string? state, string? mergedAt) =>
        ...
        state?.ToLowerInvariant() == "closed" ? MergeRequestStatus.Closed :
    ...
    private sealed class GitHubPrDto
    {
        [System.Text.Json.Serialization.JsonPropertyName("number")]
        public int Number { get; set; }
