<template>
  <div class="p-8">
    ...
    <template v-else-if="activeTab === 'Agent Runs'">
      <div v-if="store.dashboardSessions.length" class="rounded-xl border border-gray-800 overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-900">
            <tr>
              <th class="text-left px-4 py-3 text-gray-400 font-medium">Status</th>
          ...
          <tbody class="divide-y divide-gray-800">
            <tr v-for="session in store.dashboardSessions" :key="session.id"
              class="hover:bg-gray-900/50 transition-colors cursor-pointer"
