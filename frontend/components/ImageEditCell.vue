<template>
  <div class="flex items-center gap-2 min-w-0 group">
    ...
    <template v-if="!editing">
      <span
        v-if="value"
        class="text-xs font-mono text-gray-300 truncate transition-colors"
        :class="configSourceFile ? 'cursor-default' : 'cursor-pointer hover:text-white'"
        :title="value"
        @click="!configSourceFile && startEdit()"
      >{{ value }}</span>
      <span
        v-else
        class="text-xs text-gray-600 italic truncate transition-colors"
        :class="configSourceFile ? 'cursor-default' : 'cursor-pointer hover:text-gray-400'"
        :title="placeholder"
        @click="!configSourceFile && startEdit()"
      ...
      <!-- Edit button — hidden when set by config repo -->
      <button
        v-if="!configSourceFile"
